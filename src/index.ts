...
import { commands, setupCommandListeners } from './commands.ts';
import { loadSettings, saveSettings, initDB } from './libs/database.ts';
...
client.login(BOT_TOKEN).catch((error: unknown) => {
...
export async function fetchAndSendData(updatesRoleId: string, forceSend: boolean, ping: boolean) {
  try {
    ...
    // use Bun's fetch API instead of axios
    ...
    if (latestVersion !== currentVersion || forceSend) {
      ...
      if (channel.type === ChannelType.GuildAnnouncement) {
        await sentMessage.crosspost()
          ...
          .catch((error: unknown) => logMessage(LogLevel.ERROR, 'Publish Message', error));
      }
